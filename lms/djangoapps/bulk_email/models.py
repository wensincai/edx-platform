...
class CourseModeTarget(Target):
    ...
    def short_display(self):
        return "{}-{}".format(self.target_type, self.track.mode_slug)

    def long_display(self):
        all_modes = CourseMode.objects.filter(course_id=self.track.course_id, mode_slug=self.track.mode_slug)
        return "Course mode: {}, currencies: {}".format(
            self.track.mode_display_name,
