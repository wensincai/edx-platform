...
class CourseAboutLinkTestCase(CatalogIntegrationMixin, CacheIsolationTestCase):
    ...
    def setUp(self):
        ...
        self.course_cache_key = "{}{}".format(CatalogCourseRunCacheUtility.CACHE_KEY_PREFIX, self.course_key)
    ...
    def test_about_page_marketing_url_cached(self):
        catalog_course_run = {
            "key": self.course_key,
            "title": "Foo Bar Baz",
            "marketing_url": "https://marketing-url/course/course-title-foo-bar-baz"
        }
        self.assertEquals(
            get_link_for_about_page(self.course_key, self.user, catalog_course_run),
            "https://marketing-url/course/course-title-foo-bar-baz"
        )
